...
const useChatActions = () => {
  ...
  const resetTurboRefiSession = useStore(
    (state) => state.resetTurboRefiSession
  )
  const [agentId, setAgentId] = useQueryState('agent')
  const [teamId, setTeamId] = useQueryState('team')
  const [, setDbId] = useQueryState('db_id')

  const preferredAgent = (agents: AgentDetails[]) =>
    agents.find((agent) => {
      const name = (agent.name || '').toLowerCase()
      return name.includes('loa') || name.includes('loan officer')
    }) || agents[0]
      ...
      if (status === 200) {
        ...
        if (!agentId && !teamId) {
          ...
          if (currentMode === 'team' && teams.length > 0) {
            ...
            const firstAgent = preferredAgent(agents)
            setMode('agent')
