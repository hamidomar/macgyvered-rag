...
interface MessageListProps {
  ...
  onToolSelect: (toolCall: ToolCallProps['tools']) => void
          ...
          <div className="flex flex-wrap gap-2">
            {message.tool_calls.map((toolCall, index) => (
              <ToolComponent
                ...
                tools={toolCall}
                isSelected={selectedToolCallId === toolCall.tool_call_id}
                onClick={() => onToolSelect(toolCall)}
              />
            ))}
          </div>
        </div>
...
type ToolButtonProps = ToolCallProps & {
  isSelected: boolean
  onClick: () => void
}

const ToolComponent = memo(({ tools, isSelected, onClick }: ToolButtonProps) => (
  <button
    type="button"
    onClick={onClick}
    className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-left text-xs transition-colors ${
      isSelected
        ? 'border-primary/30 bg-primary/10 text-primary'
        : 'border-transparent bg-accent text-primary/80 hover:border-border hover:bg-background-secondary'
    }`}
  >
    <span className="font-dmmono uppercase">{tools.tool_name}</span>
    <Icon
